...
class PengajuanController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        foreach (['sk_pangkat', 'skp', 'ijazah', 'dokumen_pendukung'] as $field) {
            if ($request->hasFile($field)) {
                $dokumenData[$field] = $request->file($field)->store('dokumen/' . $pengajuan->id, 'public');
            }
        }
    ...
    public function updateDokumen(Request $request, Pengajuan $pengajuan)
    {
        ...
        foreach (['sk_pangkat', 'skp', 'ijazah', 'dokumen_pendukung'] as $field) {
            if ($request->hasFile($field)) {
                ...
                $dokumen->$field = $request->file($field)->store('dokumen/' . $pengajuan->id, 'public');
            }
        }
        $dokumen->save();
    ...
    public function downloadSk(Pengajuan $pengajuan)
    {
        ...
        $sekretaris = User::where('email', 'sekretaris@bkd.go.id')->first();
        $kabid = User::where('email', 'kabid@bkd.go.id')->first();

        $pdf = Pdf::loadView('admin.pengajuan.sk', compact('pengajuan', 'sekretaris', 'kabid'))
